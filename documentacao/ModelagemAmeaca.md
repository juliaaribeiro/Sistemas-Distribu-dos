...
1. Usuário → Frontend: Envia pergunta  
2. Frontend → Backend: Envia requisição HTTP  
3. Backend → Agentes de IA (Local/Externo): Processamento  
4. Agentes → Backend: Resposta processada  
5. Backend → Frontend  
6. Frontend → Usuário  
...
– Conforme matriz de risco fornecida pelo professor.
...
| 8  | Acesso não autorizado à base              | Backend → Base de Conhecimento           | Information Disclosure      | Acesso não permitido a dados internos.                                                 | 10    | 10      | 100    | Criptografia; controle de acesso; logs.                                                              | 5              | 5                 | 25              |
...
| 12 | Respostas incorretas (alucinação de IA)   | Agentes IA → Backend → Frontend          | Tampering/Integrity         | IA fornece conteúdo falso, prejudicando o cliente.                                     | 15    | 10      | 150    | Pós-validação; filtros; checagem factual; fallback seguro.                                           | 5              | 5                 | 25              |
...
| 14 | Vazamento via logs                        | Backend → Logs                            | Information Disclosure      | Logs armazenam dados sensíveis sem mascaramento.                                      | 10    | 10      | 100    | Mascaramento; redução de logs sensíveis; criptografia; controle de acesso.                          | 5              | 5                 | 25              |
...
Com os controles aplicados, o sistema torna-se muito mais seguro, mantendo **integridade**, **confidencialidade** e **disponibilidade** adequadas para um ambiente de atendimento automatizado.
